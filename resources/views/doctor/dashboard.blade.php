													...
													<div class="card-body">
														<div class="table-responsive">
															<table class="table table-hover table-center mb-0">
																...
																<tbody>
																	@if(!empty($todat_appointment))
																	 @foreach($todat_appointment as $appointment)
																	<tr>
																		<td>
																			<h2 class="table-avatar">
																				<a href="patient-profile.html" class="avatar avatar-sm mr-2"><img class="avatar-img rounded-circle" src="{{asset('patient_files')}}/{{ $appointment->profile_picture }}" alt="User Image"></a>
																				<a href="patient-profile.html">{{ $appointment->name }} <span>#PT{{ $appointment->patient_id }}</span></a>
																			</h2>
																		</td>
																		<?php $timestramp = strtotime($appointment->appointment_date); ?>
																	<?php $created_at = strtotime($appointment->created_at); ?>
																		<td><?php echo date('d F Y',$timestramp); ?> <span class="d-block text-info">{{ $appointment->appointment_slot }}</span></td>
																		<td>{{ $appointment->mobile }}</td>
																		<td>{{ $appointment->city }},{{ $appointment->state }}</td>
																		<td class="text-center">{{ $appointment->doctor_fee }} Rs.</td>
																		<td class="text-right">
																			<div class="table-action">
																				<a href="javascript:void(0);" class="btn btn-sm bg-info-light">
																					<i class="far fa-eye"></i> View
